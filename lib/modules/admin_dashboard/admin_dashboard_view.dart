...
class AdminDashboardView extends GetView<AdminDashboardController> {
        ...
        LayoutBuilder(
          builder: (context, constraints) {
            ...
            return Wrap(
              ...
              runSpacing: 24,
              children: [
                _buildStatCard('Active Users', controller.totalUsersCount, Icons.people_rounded, Colors.blue),
                _buildStatCard('Pending Tickets', controller.totalTicketsCount, Icons.forum_rounded, Colors.orange),
                _buildStatCard('Total Banners', controller.totalBannersCount, Icons.collections_rounded, Colors.purple),
                _buildStatCard('Admin Accounts', controller.totalAdminsCount, Icons.shield_rounded, Colors.green),
              ],
            );
          },
        ),
  ...
  Widget _buildStatCard(String label, RxInt count, IconData icon, Color color) {
    return Container(
      width: 240,
  ...
  Widget _buildPriorityBadge(TicketPriority priority) {
    ...
    return Container(
      ...
      child: Text(priority.name.toUpperCase(), style: TextStyle(color: color, fontSize: 9, fontWeight: FontWeight.bold)),
    );
  }
  ...
  void _openAdminChat(String ticketId, String userName) {
    Get.dialog(
      Dialog.fullscreen(
        child: _AdminChatView(ticketId: ticketId, userName: userName),
      ),
      useSafeArea: false,
    );
  }
        ...
        _buildFormCard([
          ...
          Obx(() => Wrap(
            ...
            children: [
              ...
              _buildExpiryChip(context, controller, 720, '30 days', isAd: true),
          ...
          const SizedBox(height: 24),
          Obx(() => _buildActionButton(
            controller.isAdEditMode.value ? 'Update Campaign' : 'Launch Campaign',
            controller.addCustomAd,
            isLoading: controller.isAdSubmitting.value,
          )),
          if (controller.isAdEditMode.value)
            TextButton(onPressed: controller.cancelAdEdit, child: const Text('Cancel Edit', style: TextStyle(color: Colors.grey))),
        ]),
        const SizedBox(height: 40),
        const Text('Active Ad Campaigns', style: TextStyle(color: Colors.white70, fontSize: 18, fontWeight: FontWeight.bold)),
        const SizedBox(height: 16),
        Obx(() {
          if (controller.customAds.isEmpty) return const Center(child: Text('No active ads found', style: TextStyle(color: Colors.white24)));
          return ListView.separated(
            shrinkWrap: true,
            physics: const NeverScrollableScrollPhysics(),
            ...
            itemBuilder: (context, index) {
              ...
              return _buildItemCard(
                data['imageUrl'],
                data['title'] ?? 'No Title',
                data['targetUrl'] ?? 'No Target',
                () => controller.enterAdEditMode(id, data),
                () => controller.deleteCustomAd(id),
                data: data,
                trailing: Switch(
                  value: status == 'active',
                  onChanged: (val) => controller.toggleAdStatus(id, status),
    ...
    return Column(
      ...
      children: [
        ...
        _buildFormCard([
          ...
          Obx(() {
            if (n8nController.testResultText.value.isEmpty && n8nController.testResultStatus.value == null) {
            ...
            return Column(
              ...
              children: [
                ...
                Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    const Text('Test Result:', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.white)),
                    if (status != null)
                      Container(
                        ...
                        child: Text('HTTP $status', style: TextStyle(color: badgeColor, fontSize: 12, fontWeight: FontWeight.bold)),
                      ),
                  ],
                ),
                const SizedBox(height: 8),
                Container(
                  ...
                  child: Text(
                    ...
                    style: TextStyle(color: isSuccess ? Colors.white : Colors.redAccent, fontSize: 13.5, height: 1.4),
                  ),
                ),
                const SizedBox(height: 16),
                Theme(
                        ...
                        child: SingleChildScrollView(
                          ...
                          child: SingleChildScrollView(
                            ...
                            child: Text(
                              ...
                              style: GoogleFonts.sourceCodePro(color: Colors.greenAccent, fontSize: 11.5),
                            ),
                          ),
                        ),
                      ),
  ...
  Widget _buildTextField(TextEditingController controller, String label, IconData icon, {int maxLines = 1}) {
    return TextField(
      ...
      decoration: InputDecoration(
        ...
        border: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide.none),
        focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: const BorderSide(color: Color(0xFF1B5E35), width: 1)),
      ),
    );
  }
  ...
  Widget _buildActionButton(String label, VoidCallback onPressed, {bool isLoading = false}) {
    return ElevatedButton(
      ...
      child: isLoading ? const SizedBox(width: 20, height: 20, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white)) : Text(label, style: const TextStyle(fontWeight: FontWeight.bold)),
    );
  }
}
...
class _AdminChatView extends StatefulWidget {
  ...
  const _AdminChatView({required this.ticketId, required this.userName});
...
class _AdminChatViewState extends State<_AdminChatView> {
  late AdminChatController chatController;
  ...
  @override
  void dispose() {
    Get.delete<AdminChatController>(tag: widget.ticketId);
    super.dispose();
  }
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          Expanded(
            child: Obx(() {
              if (chatController.messages.isEmpty) return const Center(child: Text('No messages yet', style: TextStyle(color: Colors.white12)));
              return ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  return _AdminMessageBubble(message: msg, isMe: msg.senderId == 'admin');
                },
              );
            }),
          ),
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          const SizedBox(width: 8),
          IconButton(
            icon: const Icon(Icons.send_rounded, color: Color(0xFF1B5E35)),
      ...
      child: Column(
        ...
        children: [
          Container(
            ...
            padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12),
            decoration: BoxDecoration(
              color: isMe ? const Color(0xFF1B5E35) : const Color(0xFF252538),
            ...
            child: Column(
              ...
              children: [
                ...
                if (message.message.isNotEmpty)
                  Text(message.message, style: const TextStyle(color: Colors.white, fontSize: 14)),
              ],
            ),
          ),
          Text(DateFormat('hh:mm a').format(message.timestamp), style: const TextStyle(fontSize: 10, color: Colors.white30)),
        ],
...
class _StarPatternPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    final paint = Paint()..color = Colors.white..style = PaintingStyle.stroke..strokeWidth = 0.8;
    const step = 32.0;
